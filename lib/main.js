...
LambdaRouter.prototype.isValidLambda = function(lambda, functionName, env, version) {
...
LambdaRouter.prototype.findMatchingLambda = function(functionName, env, version, region, cb) { 
  var that = this;
  ...
  lambda.listFunctions({}, function(err, data) {
    ...
    var err = new Error('No lambda matches found for' + functionName + '-' + env + '-' + version + ' in ' + region);
    return cb(err, null); // No matches found
  });
};
...
LambdaRouter.prototype.invokeFallback = function(lambdas, payload, callback) {
  var that = this;
...
LambdaRouter.prototype.invokeAsync = function(functionName, env, version, regions, payload, callback) {
  ...
  async.map(regions, async.apply(that.findMatchingLambda.bind(that), functionName, env, version), function(err, results) {
    if (err) {
      return callback(err);
    }
